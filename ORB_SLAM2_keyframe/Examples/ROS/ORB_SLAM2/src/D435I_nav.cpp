...
int main(int argc, char **argv)
{
    ...
    {
        string orb_map = MapPath + ".bin";
        string pointcloud_map = MapPath + ".pcd";
        SLAM.SaveMap(orb_map.c_str());
        SLAM.SavePointCloud(pointcloud_map.c_str());
        cout<<"Map Save in "<<MapPath.c_str()<<endl;
    }
    ros::shutdown();
    return 0;
}
...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD)
{
    ...
    Tcw = mpSLAM->TrackRGBD(cv_ptrRGB->image,cv_ptrD->image,cv_ptrRGB->header.stamp.toSec(),isKeyFrame,PointCloudMapping);
    if(PointCloudMapping)
    {
    ...
    if (!Tcw.empty())
    {
        ...
        odom_trans.header.frame_id = "orb";
